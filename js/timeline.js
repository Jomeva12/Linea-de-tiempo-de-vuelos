 ...
 * Bug: cannot set max width for an item, like div.timeline-event-content {white-space: normal; max-width: auto;}
...
links.Timeline = function (container, options) {
  ...
  this.options = {
    ...
    zoomMax: 1000 * 60 * 60 * 24 * 365, // milliseconds
    //1000 * 60 * 60 * 24 * 365 * 10000,
    moveable: true,
    zoomable: true,
    selectable: true,
    unselectable: true,
    ...
    DAYS_SHORT: ["Sun", "Mon", "Mar", "Wed", "Thu", "Fri", "Sat"],
    ZOOM_IN: "Zoom in",
    ZOOM_OUT: "Zoom out",
    MOVE_LEFT: "Move left",
...
links.Timeline.prototype.setOptions = function (options) {
  if (options) {
    ...
    if (options.showButtonAdd != undefined) {
      ...
      console.log(
        "WARNING: Option showButtonAdd is deprecated. Use showButtonNew instead"
      );
    }
    if (options.intervalMin != undefined) {
      this.options.zoomMin = options.intervalMin;
      console.log(
        "WARNING: Option intervalMin is deprecated. Use zoomMin instead"
      );
    }
    if (options.intervalMax != undefined) {
      this.options.zoomMax = options.intervalMax;
      console.log(
        "WARNING: Option intervalMax is deprecated. Use zoomMax instead"
      );
    }
...
links.Timeline.prototype.setData = function (data) {
  ...
  if (
    ...
    var zoomAroundDate = localStorage.getItem("zoomAroundDate");
    var interval = localStorage.getItem("interval");
    var newStart = localStorage.getItem("newStart");
    var newEnd = localStorage.getItem("newEnd");
    var zoomMax = localStorage.getItem("zoomMax");

    //this.options.zoomMax= 1000 * 60 * 60 *5 
    this.options.zoomMax= interval 
    //console.log(newStart)
    // if (interval >= zoomMax) {
    //     console.log(newStart,newEnd,zoomAroundDate)
    ...
    for (var row = 0, rows = data.getNumberOfRows(); row < rows; row++) {
      items.push(
        this.createItem({
          start:
            cols.start != undefined
              ? data.getValue(row, cols.start)
              : undefined,
          end: cols.end != undefined ? data.getValue(row, cols.end) : undefined,
          content:
            cols.content != undefined
              ? data.getValue(row, cols.content)
              : undefined,
          group:
            cols.group != undefined
              ? data.getValue(row, cols.group)
              : undefined,
          className:
            cols.className != undefined
              ? data.getValue(row, cols.className)
              : undefined,
          editable:
            cols.editable != undefined
              ? data.getValue(row, cols.editable)
              : undefined,
          type:
            cols.type != undefined ? data.getValue(row, cols.type) : undefined,
        })
      );
...
links.Timeline.prototype.getItemIndex = function (element) {
  var e = element,
    ...
    items = this.items,
    index = undefined;
  // console.log(item.group.content)
...
links.Timeline.prototype.setVisibleChartRangeNow = function () {
  ...
  console.log("jomeva entrando",startNew,endNew,diff)
...
links.Timeline.prototype.getDataRange = function (withMargin) {
  ...
  if (items) {
    for (var i = 0, iMax = items.length; i < iMax; i++) {
      ...
      if (start != undefined) {
        min =
          min != undefined ? Math.min(min.valueOf(), start.valueOf()) : start;
      }
...
links.Timeline.prototype.repaint = function () {
  ...
  return (
    frameNeedsReflow || axisNeedsReflow || groupsNeedsReflow || itemsNeedsReflow
  );
};
...
links.Timeline.prototype.repaintFrame = function () {
  ...
  if (!dom.frame) {
    ...
    dom.frame.className =
      "timeline-frame ui-widget ui-widget-content ui-corner-all";
  ...
  if (!dom.content) {
    ...
    if (!params.onMouseDown) {
      ...
      links.Timeline.addEventListener(
        dom.content,
        "mousedown",
        params.onMouseDown
      );
...
links.Timeline.prototype.reflowAxis = function () {
  ...
  size.axis.labelMajorTop = options.axisOnTop
    ? 0
    : axisLine + (options.showMinorLabels ? characterMinorHeight : 0);
  size.axis.labelMinorTop = options.axisOnTop
    ? options.showMajorLabels
      ? characterMajorHeight
      : 0
    : axisLine;
  ...
  if (groups.length == 0) {
    ...
    for (i = 0, iMax = groups.length; i < iMax; i++) {
      ...
      if (options.axisOnTop) {
        ...
        group.labelTop =
          top + axisHeight + (group.height - group.labelHeight) / 2;
...
links.Timeline.prototype.reflowGroups = function () {
  ...
  if (options.groupsWidth !== undefined) {
    groupsWidth =
      dom.groups && dom.groups.frame ? dom.groups.frame.clientWidth : 0;
  }
...
links.Timeline.prototype.repaintGroups = function () {
  ...
  links.Timeline.addClassName(
    frame,
    options.groupsOnRight
      ? "timeline-groups-axis-onright"
      : "timeline-groups-axis-onleft"
  );

  // position the groups
  var vector = [74, 94, 134, 174, 214];
  //var vector = [54, 94, 134, 174, 214]
  for (var i = 0, iMax = groups.length; i < iMax; i++) {
    var group = groups[i],
      label = labels[i],
      labelLine = labelLines[i],
...
links.Timeline.prototype.repaintDeleteButton = function () {
  ...
  if (!deleteButton) {
    ...
    // boton borrar para que no aparezca
  ...
  var index =
      this.selection && this.selection.index !== undefined
...
//jomeva
links.Timeline.prototype.repaintConfigButton = function () {
  var timeline = this,
    dom = this.dom,
    frame = dom.items.frame;

  var configButton = dom.items.configButton;
  if (!configButton) {
    //jomeva
    configButton = document.createElement("DIV");
    configButton.className = "timeline-navigation-config";
    configButton.style.position = "relative";
    configButton.setAttribute("data-bs-target", "#modalVuelos");
    configButton.setAttribute("data-bs-toggle", "modal");
    //configButton.attr("data-bs-toggle"="modal")
    // data-bs-target="#modalVuelos"
    frame.appendChild(configButton);
    dom.items.configButton = configButton;
  }
  ...
  if (item && item.rendered && this.isEditable(item)) {
    ...
    configButton.style.left = right + "px";
    configButton.style.top = top + "px";
    configButton.style.display = "";
    frame.removeChild(configButton);
    frame.appendChild(configButton);
  } else {
...
links.Timeline.prototype.repaintDragAreas = function () {
  ...
  if (
    ...
    (item instanceof links.Timeline.ItemRange ||
      item instanceof links.Timeline.ItemFloatingRange)
  ) {
...
links.Timeline.prototype.repaintNavigation = function () {
  ...
  if (!navBar) {
    ...
    var showNavigation =
      options.showNavigation && (options.zoomable || options.moveable);
    if (showNavigation || showButtonNew) {
      // create a navigation bar containing the navigation buttons
      navBar = document.createElement("DIV");
      navBar.style.position = "absolute";
      navBar.className =
        "timeline-navigation ui-widget ui-state-highlight ui-corner-all";
    ...
    if (showNavigation) {
      ...
      if (options.moveable) {
        ...
        navBar.appendChild(navBar.moveRightButton);
          ...
          // create a update
          navBar.upDate = document.createElement("DIV");
          navBar.upDate.className = "timeline-navigation-move-upDate";
          navBar.upDate.title = this.options.MOVE_LEFT;
          var mlIconSpan = document.createElement("SPAN");
          mlIconSpan.className = "ui-icon ui-icon-circle-arrow-w";
          navBar.upDate.appendChild(mlIconSpan);
  
          var onMoveupDate = function (event) {
            links.Timeline.preventDefault(event);
            links.Timeline.stopPropagation(event);
           //jomeva actualizar
           tableToJSON();
          ...
          navBar.appendChild(navBar.upDate);
           // create a centrar
           navBar.centrar = document.createElement("DIV");
           navBar.centrar.className = "timeline-navigation-move-centrar";
           navBar.centrar.title = this.options.MOVE_LEFT;
           var mlIconSpan = document.createElement("SPAN");
           mlIconSpan.className = "ui-icon ui-icon-circle-arrow-w";
           navBar.centrar.appendChild(mlIconSpan);
   
           var onMovecentrar = function (event) {
             links.Timeline.preventDefault(event);
             links.Timeline.stopPropagation(event);
            //jomeva actualizar
           ...
           navBar.appendChild(navBar.centrar);
      }
    }
  }
};
...
links.Timeline.prototype.recalcConversion = function () {
  ...
  this.conversion.factor =
    this.size.contentWidth / (this.end.valueOf() - this.start.valueOf());
};
...
links.Timeline.prototype.onTouchEnd = function (event) {
  ...
  if (params.onTouchMove) {
    links.Timeline.removeEventListener(
      document,
      "touchmove",
      params.onTouchMove
    );
...
links.Timeline.prototype.onMouseDown = function (event) {
  ...
  var dragLeft =
    dom.items && dom.items.dragLeft ? dom.items.dragLeft : undefined;
  var dragRight =
    dom.items && dom.items.dragRight ? dom.items.dragRight : undefined;
...
/*metodo para buscar en la tabla y obtener los datos de la fila para actualizar*/
function getFila(fila, option, puente,salaCambiada) {
    ...
    .each(function () {
      ...
      if (codigo == fila) {
        ...
        if (option == "move") {
          ...
          trDelResultado.find("td:eq(10)").html(salaCambiada);
          trDelResultado.find("td:eq(5)").html(puente)
         // 
        //  var zoomMin = Number(this.options.zoomMin) || 10;
    var zoomAroundDate = localStorage.getItem("zoomAroundDate");
    var interval = localStorage.getItem("interval");
  ...
  //  links.Timeline.prototype.setVisibleChartRangeauto();
          ...
          $("#vueloLlegada").val(vueloLlegada);
          $("#vueloSalida").val(vueloSalida);
...
links.Timeline.prototype.onMouseMove = function (event) {
  ...
  if (params.customTime) {
    ...
    if (this.groups.length && !dragging) {
      ...
      var group = this.getGroupFromHeight(y);
      // console.log("jomeva cambiando "+this.getGroupName(group))
      ...
      if (options.groupsChangeable && item.group !== group) {
        ...
        var puente = "";
        var sala = "";
        switch (this.getGroupName(group)) {
          case "Puente 1": {
            puente = "P1";
            sala = "1";
            break;
        ...
        getFila(this.selection.index, "move", puente,sala);
      } else {
        this.repaintDeleteButton();
        this.repaintDragAreas();
        this.repaintConfigButton();
    ...
    var frameLeft =
      (-diffMillisecs / interval) * size.contentWidth + frameOffset;
...
links.Timeline.prototype.onMouseUp = function (event) {
  ...
  if (params.customTime) {
    ...
    if (!params.moved && !params.zoomed) {
      ...
      } else if (params.target === this.dom.items.configButton) {
        // delete item
        if (this.selection && this.selection.index !== undefined) {
          getFila(this.selection.index, "clic", "","");
      ...
      if (
        (params.moved && options.moveable) ||
        (params.zoomed && options.zoomable)
      ) {
...
links.Timeline.prototype.zoom = function (zoomFactor, zoomAroundDate) {
  ...
  //-801645 436541 interval 1643643
  ...
  localStorage.setItem("newStart", newStart);
  localStorage.setItem("newEnd", newEnd);
  localStorage.setItem("zoomAroundDate", zoomAroundDate);
 // localStorage.setItem("zoomMax", zoomMax);
  var zoomMin = Number(this.options.zoomMin) || 10;
  //var zoomMax = Number(this.options.zoomMax) || 10000 * year;
   //jomeva con esto reescribo el max y arrriba lo asigno a interval
  this.options.zoomMax=1000 * 60 * 60 * 24 * 365
...
links.Timeline.prototype.applyRange = function (start, end, zoomAroundDate) {
  ...
  if (interval < zoomMin) {
    ...
    var f = zoomAroundDate
      ? (zoomAroundDate.valueOf() - startValue) / interval
      : 0.5;
...
links.Timeline.ItemBox.prototype.select = function () {
  ...
  links.Timeline.addClassName(
    dom.line,
    "timeline-event-selected ui-state-active"
  );
  links.Timeline.addClassName(
    dom.dot,
    "timeline-event-selected ui-state-active"
...
links.Timeline.ItemBox.prototype.updateDOM = function () {
  ...
  if (divBox) {
    ...
    // update class
    divBox.className =
      "timeline-event timeline-event-box ui-widget ui-state-default";
    divLine.className =
      "timeline-event timeline-event-line ui-widget ui-state-default";
    divDot.className =
      "timeline-event timeline-event-dot ui-widget ui-state-default";

    if (this.isCluster) {
      links.Timeline.addClassName(
        divBox,
        "timeline-event-cluster ui-widget-header"
      );
      links.Timeline.addClassName(
        divLine,
        "timeline-event-cluster ui-widget-header"
...
links.Timeline.ItemBox.prototype.updatePosition = function (timeline) {
  ...
  if (dom) {
    var left = timeline.timeToScreen(this.start),
      ...
      boxAlign =
        timeline.options.box && timeline.options.box.align
          ? timeline.options.box.align
          : undefined;
...
links.Timeline.ItemRange.prototype.updateDOM = function () {
  ...
  if (divBox) {
    ...
    divBox.className =
      "timeline-event timeline-event-range ui-widget ui-state-default";
    if (this.isCluster) {
      links.Timeline.addClassName(
        divBox,
        "timeline-event-cluster ui-widget-header"
 ...
 * @override
 */
links.Timeline.ItemFloatingRange.prototype.updatePosition = function (
  timeline
) {
  var dom = this.dom;
  if (dom) {
 ...
 * @override
 */
links.Timeline.ItemFloatingRange.prototype.setPosition = function (
  left,
  right
) {
  var dom = this.dom;
...
links.Timeline.prototype.createItem = function (itemData) {
  ...
  if (options.axisOnTop) {
    initialTop =
      this.size.axis.height + options.eventMarginAxis + options.eventMargin / 2;
  } else {
    initialTop =
      this.size.contentHeight -
      options.eventMarginAxis -
      options.eventMargin / 2;
  }
 ...
 * @param {boolean} [preventRender=false]   Do not re-render timeline if true
 */
links.Timeline.prototype.changeItem = function (
  index,
  itemData,
  preventRender
) {
  ...
  var newItem = this.createItem({
    ...
    content: itemData.hasOwnProperty("content")
      ? itemData.content
      : oldItem.content,
    group: itemData.hasOwnProperty("group")
      ? itemData.group
      : this.getGroupName(oldItem.group),
    className: itemData.hasOwnProperty("className")
      ? itemData.className
      : oldItem.className,
    editable: itemData.hasOwnProperty("editable")
      ? itemData.editable
      : oldItem.editable,
...
links.Timeline.prototype.stackOrder = function (items) {
  ...
  var f =
    ...
    typeof this.options.customStackOrder === "function"
      ? this.options.customStackOrder
      : function (a, b) {
          if (
            (a instanceof links.Timeline.ItemRange ||
              a instanceof links.Timeline.ItemFloatingRange) &&
            !(
              b instanceof links.Timeline.ItemRange ||
              b instanceof links.Timeline.ItemFloatingRange
            )
          ) {
            return -1;
          }
...
links.Timeline.prototype.stackCalculateFinal = function (items) {
  ...
  for (j = 0; j < this.groups.length; ++j) {
    ...
    groupFinalItems = this.finalItemsPosition(
      groupedItems[group.content],
      groupBase,
      group
    );
...
links.Timeline.prototype.finalItemsPosition = function (
  items,
  groupBase,
  group
) {
    ...
    if (this.options.stackEvents) {
      do {
        ...
        if (collidingItem != null) {
          collidingItem.item.dom.className = "timeline-colision";

          // There is a collision. Reposition the event above the colliding element
          if (axisOnTop) {
            finalItem.top =
              collidingItem.top + collidingItem.height + eventMargin;
          } else {
  ...
  //console.log(finalItem.item.content)
 ...
 *                               location, else false
 */
links.Timeline.prototype.stackMoveOneStep = function (
  currentItems,
  finalItems
) {
  var arrived = true;
 ...
 *                               positions of the items
 */
links.Timeline.prototype.stackMoveToFinal = function (
  currentItems,
  finalItems
) {
...
links.Timeline.prototype.clusterItems = function () {
  ...
  var clusters = this.clusterGenerator.getClusters(
    this.conversion.factor,
 ...
 */
links.Timeline.ClusterGenerator.prototype.getClusters = function (
  scale,
  maxItems
) {
  var level = -1,
          ...
          if (neighbors > maxItems) {
            ...
            while (clusterItems.length < num && m < items.length) {
              ...
              if (count) {
                ...
                avg =
                  (count / (count + 1)) * avg + (1 / (count + 1)) * p.center;
              } else {
                avg = p.center;
              }
              ...
              containsRanges =
                containsRanges ||
                p instanceof links.Timeline.ItemRange ||
                p instanceof links.Timeline.ItemFloatingRange;
              count++;
              m++;
            }
            ...
            var content =
              '<div title="' + title + '">' + count + " events</div>";
 ...
 */
links.Timeline.StepDate.prototype.setRange = function (
  start,
  end,
  minimumStep
) {
  ...
  switch (this.scale) {
    case links.Timeline.StepDate.SCALE.YEAR:
      this.current.setFullYear(
        this.step * Math.floor(this.current.getFullYear() / this.step)
      );
  ...
  if (this.step != 1) {
    ...
    switch (this.scale) {
      case links.Timeline.StepDate.SCALE.MILLISECOND:
        this.current.setMilliseconds(
          this.current.getMilliseconds() -
            (this.current.getMilliseconds() % this.step)
        );
        break;
      case links.Timeline.StepDate.SCALE.SECOND:
        this.current.setSeconds(
          this.current.getSeconds() - (this.current.getSeconds() % this.step)
        );
        break;
      case links.Timeline.StepDate.SCALE.MINUTE:
        this.current.setMinutes(
          this.current.getMinutes() - (this.current.getMinutes() % this.step)
        );
        break;
      case links.Timeline.StepDate.SCALE.HOUR:
        this.current.setHours(
          this.current.getHours() - (this.current.getHours() % this.step)
        );
        break;
      ...
      case links.Timeline.StepDate.SCALE.DAY:
        this.current.setDate(
          this.current.getDate() -
            1 -
            ((this.current.getDate() - 1) % this.step) +
            1
        );
        break;
      case links.Timeline.StepDate.SCALE.MONTH:
        this.current.setMonth(
          this.current.getMonth() - (this.current.getMonth() % this.step)
        );
        break;
      case links.Timeline.StepDate.SCALE.YEAR:
        this.current.setFullYear(
          this.current.getFullYear() - (this.current.getFullYear() % this.step)
        );
        break;
      default:
        break;
...
links.Timeline.StepDate.prototype.next = function () {
  ...
  if (this.current.getMonth() < 6) {
    switch (this.scale) {
      ...
      case links.Timeline.StepDate.SCALE.HOUR:
        this.current = new Date(
          this.current.valueOf() + this.step * 1000 * 60 * 60
        );
 ...
 */
links.Timeline.addEventListener = function (
  element,
  action,
  listener,
  useCapture
